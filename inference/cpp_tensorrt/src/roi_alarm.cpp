...
std::vector<std::string> ExtractStringArray(const std::string& text, const std::string& key) {
    ...
    const std::string array_text = ExtractSection(text, key, '[', ']');
    if (!array_text.empty()) {
        const std::regex string_pattern("\\\"([^\\\"]*)\\\"");
        for (std::sregex_iterator it(array_text.begin(), array_text.end(), string_pattern), end; it != end; ++it) {
            values.push_back((*it)[1].str());
        }
        return values;
    }

    // 兼容单类别写法："class_name": "person"
    const std::string single_value = ExtractString(text, key, "");
...
bool LoadSafetyConfig(
    ...
    inference_config.score_threshold = ExtractFloat(config_text, "conf_thres", inference_config.score_threshold);
    inference_config.iou_threshold = ExtractFloat(config_text, "iou_thres", inference_config.iou_threshold);
    inference_config.topk = ExtractInt(config_text, "topk", inference_config.topk);

    inference_config.class_names = ExtractStringArray(config_text, "class_name");
    if (!inference_config.class_names.empty()) {
        inference_config.num_labels = static_cast<int>(inference_config.class_names.size());
    } else {
        // 没有配置 class_name 时，后处理会根据 TensorRT 输出通道数自动推断类别数。
        inference_config.num_labels = ExtractInt(config_text, "num_labels", inference_config.num_labels);
    }
