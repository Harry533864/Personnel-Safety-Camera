...
STREAM_HIGH_PUBLISH="${STREAM_HIGH_PUBLISH:-0}"
...
Environment=CAMERA_SOURCE=$CAMERA_SOURCE
...
Environment=CAMERA_FOURCC=$CAMERA_FOURCC
Environment=USB_CAMERA_DEVICE=$USB_CAMERA_DEVICE
Environment=USB_CAMERA_WIDTH=$USB_CAMERA_WIDTH
Environment=USB_CAMERA_HEIGHT=$USB_CAMERA_HEIGHT
Environment=USB_CAMERA_FPS=$USB_CAMERA_FPS
Environment=USB_CAMERA_FOURCC=$USB_CAMERA_FOURCC
Environment=USB_CAMERA_DECODER=$USB_CAMERA_DECODER
Environment=CSI_SENSOR_ID=$CSI_SENSOR_ID
Environment=CSI_FLIP_METHOD=$CSI_FLIP_METHOD
Environment=CSI_CAMERA_WIDTH=$CSI_CAMERA_WIDTH
Environment=CSI_CAMERA_HEIGHT=$CSI_CAMERA_HEIGHT
Environment=CSI_CAMERA_FPS=$CSI_CAMERA_FPS
Environment=CSI_READY_DEVICE=$CSI_READY_DEVICE
Environment=CSI_DIRECT_STREAM=$CSI_DIRECT_STREAM
Environment=CSI_DIRECT_RTMP_URL=$CSI_DIRECT_RTMP_URL
Environment=CSI_DIRECT_BITRATE_KBPS=$CSI_DIRECT_BITRATE_KBPS
Environment=CSI_DIRECT_KEY_INT=$CSI_DIRECT_KEY_INT
Environment=CSI_DIRECT_VBV_MS=$CSI_DIRECT_VBV_MS
Environment=STREAM_HIGH_PUBLISH=$STREAM_HIGH_PUBLISH
Environment=STREAM_HIGH_RECORD=$STREAM_HIGH_RECORD
Environment=STREAM_HIGH_WIDTH=$STREAM_HIGH_WIDTH
Environment=STREAM_HIGH_HEIGHT=$STREAM_HIGH_HEIGHT
Environment=STREAM_HIGH_FPS=$STREAM_HIGH_FPS
Environment=STREAM_ENCODER=$STREAM_ENCODER
Environment=STREAM_OPENH264_THREADS=$STREAM_OPENH264_THREADS
Environment=STREAM_X264_THREADS=$STREAM_X264_THREADS
Environment=STREAM_X264_SLICED_THREADS=$STREAM_X264_SLICED_THREADS
Environment=CAM_AUTO_START=1
Environment=FLASK_DEBUG=0
Environment=PYTHONUNBUFFERED=1
Environment=PYTHON_BIN=$PYTHON_BIN
ExecStartPre=/bin/sh -c 'ready="\$CAMERA_DEVICE"; if [ "\$CAMERA_SOURCE" = "csi" ]; then ready="\$CSI_READY_DEVICE"; fi; i=0; while [ ! -e "\$ready" ] && [ "\$i" -lt "\$CAMERA_WAIT_SECONDS" ]; do i=\$((i+1)); sleep 1; done; exit 0'
ExecStart=/bin/sh -c 'if "\$PYTHON_BIN" -m gunicorn --version >/dev/null 2>&1; then exec "\$PYTHON_BIN" -m gunicorn --workers 1 --threads 4 --bind "\$FLASK_HOST:\$FLASK_PORT" app:app; else exec "\$PYTHON_BIN" -m flask --app app run --host="\$FLASK_HOST" --port="\$FLASK_PORT"; fi'
Restart=always
RestartSec=3
