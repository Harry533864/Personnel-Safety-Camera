          ...
          <div class="panel-toolbar">
            ...
            <div class="toolbar-actions">
              ...
              <select v-model="selectedTarget" class="target-select" :disabled="isSaving">
                <option
                  v-for="option in roiResolutionOptions"
                  :key="option.value"
                  :value="option.value"
                >
                  {{ option.label }}
...
function isResolutionKey(value) {
  return /^\d+x\d+$/.test(String(value || "").trim().toLowerCase().replace(/\s+/g, ""));
}

function normalizeResolutionKey(value) {
  const key = String(value || "").trim().toLowerCase().replace(/\s+/g, "");
  return isResolutionKey(key) ? key : DEFAULT_ROI_RESOLUTION;
}

function formatResolutionLabel(value) {
  const [width, height] = normalizeResolutionKey(value).split("x");
  return `${width} x ${height}`;
}

function getMaxFpsForResolution(value) {
  const key = normalizeResolutionKey(value);
  return roiResolutionOptions.value.find((option) => option.value === key)?.maxFps
    || Number(cameraSettingStore.settings.fps || 60)
    || 60;
}
...
const roiResolutionOptions = computed(() => {
  const options = BASE_RESOLUTION_OPTIONS.map((option) => ({ ...option }));
  const seen = new Set(options.map((option) => option.value));
  const currentFps = Number(cameraSettingStore.settings.fps || 60) || 60;

  [
    selectedTarget.value,
    cameraSettingStore.settings.resolution,
    detectionRegionStore.getCurrentTarget(),
  ].forEach((value) => {
    const key = normalizeResolutionKey(value);
    if (!isResolutionKey(key) || seen.has(key)) return;
    options.unshift({
      value: key,
      label: `${formatResolutionLabel(key)} (current)`,
      maxFps: currentFps,
    });
    seen.add(key);
  });
...
const savedSourceResolution = computed(() => {
  const value = selectedTarget.value || cameraSettingStore.settings.resolution || DEFAULT_ROI_RESOLUTION;
...
async function syncRegionsFromServer() {
  ...
  const currentTarget = detectionRegionStore.getCurrentTarget();
  if (currentTarget && currentTarget !== selectedTarget.value) {
